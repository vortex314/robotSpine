...
CborReader &CborReader::close()
{
	if (_error == CborNoError && _containers.size() > 0)
	{
		...
		while (cbor_value_get_type(_value) != CborInvalidType && !cbor_value_at_end(_value))
...
{
/*	json = cborDump(Bytes(_data, _data + _size));*/
	_error =
		cbor_value_to_pretty_stream(stringStreamFunction, &json, _value, CborPrettyNumericEncodingIndicators);
		if ( hasEnding(json,".") ) json+="0";
	return *this;
}
