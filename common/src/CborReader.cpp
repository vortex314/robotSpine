...
CborReader::CborReader(size_t capacity) : _capacity(capacity) {
  _data = (uint8_t *)(new uint32_t[(capacity / 4) + 1]);
  _value = new CborValue;
}

CborReader::~CborReader() { delete _data; }
...
CborReader &CborReader::toJson(std::string &json) {
  CborStreamFunction stream = stringStreamFunction;
  _error = cbor_value_to_pretty_stream(stringStreamFunction, &json, _value, 0);
